...
public class OrientationUtils2 {
     ...
     */
    public OrientationUtils2(Activity activity, MediaController mediaController) {
    ...
    private void init() {
        orientationEventListener = new OrientationEventListener(mContent) {
            @Override
            public void onOrientationChanged(int rotation) {
                ...
                if (((rotation >= 0) && (rotation <= 30)) || (rotation >= 330)) {
                    if (mClick) {
                        ...
                        if (mIsLand > 0) {
                            ...
                            // mContent.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                            if (mediaController.isFullscreen()) {
                                mediaController.getFullscreenButton().setImageResource(mediaController.getShrinkImageRes());
                            } else {
                                mediaController.getFullscreenButton().setImageResource(mediaController.getEnlargeImageRes());
                            }
                            mIsLand = 0;
                            mClick = false;
                        }
                ...
                // 设置横屏
                    ...
                    if (mClick) {
                        ...
                        if (!(mIsLand == 1)) {
                            ...
                            setScreenOrientation(screenType);
                            // mContent.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
                            mediaController.getFullscreenButton().setImageResource(mediaController.getShrinkImageRes());
                            mIsLand = 1;
                            mClick = false;
                        }
                ...
                // 设置反向横屏
                    ...
                    if (mClick) {
                        ...
                        screenType = ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE;
                        setScreenOrientation(screenType);
                        // mContent.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_REVERSE_LANDSCAPE);
                        mediaController.getFullscreenButton().setImageResource(mediaController.getShrinkImageRes());
                        mIsLand = 2;
                        mClick = false;
                    }
    ...
    public OrientationUtils2 setEnable(boolean enable) {
        this.mEnable = enable;
        if (mEnable) {
            orientationEventListener.enable();
        } else {
     ...
     */
    public OrientationUtils2 setRotateWithSystem(boolean rotateWithSystem) {
    ...
    private void setScreenOrientation(int screenOrientation) {
        ...
        if (mCallback != null) {
            mCallback.screenOrientationChangle(screenOrientation);
        }
    }

    public OrientationUtils2 setCallback(Callback callback) {
        mCallback = callback;
        return this;
    }
