...
public class AliyunVodPlayerView extends RelativeLayout implements ITheme {
    ...
    //皮肤view
    public static ControlView mControlView;
    //清晰度view
...
//先隐藏手势和控制栏，防止在没有prepare的时候做操作。
    ...
    public static class InnerOrientationListener implements OrientationWatchDog.OnOrientationListener {
        ...
        @Override
        public void changedToLandScape(boolean fromPort) {//全屏
        ...
        @Override
        public void changedToPortrait(boolean fromLand) {//竖屏
            AliyunVodPlayerView playerView = playerViewWeakReference.get();
            if (playerView != null) {
...
//                }
                if (mAliyunMediaInfo != null && mAliyunVodPlayer.getCurrentQuality() != null && mControlView != null) {
                    mControlView.setMediaInfo(mAliyunMediaInfo, mAliyunVodPlayer.getCurrentQuality());
                    setCoverUri(mAliyunMediaInfo.getPostUrl());
                }
    ...
    public void changeScreenMode(AliyunScreenMode targetMode) {
        ...
        if (context instanceof Activity) {
            if (finalScreenMode == AliyunScreenMode.Full) {
                ...
                //add
                Intent intent = new Intent("ScreenMode");
                intent.putExtra("model","full");
